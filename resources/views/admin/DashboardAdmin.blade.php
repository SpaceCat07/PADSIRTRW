    ...
    <div class="admin-dashboard-container">
        ...
        <div class="header-container">
            <header class="dashboard-header">
                <div class="header-admin-left">
                    <div class="toggle-sidebar-icon"><img src="{{ asset('storage/sidebarIcon.png') }}" alt="Toggle Sidebar">
                    </div>
                ...
                <div class="header-admin-dropdown text-end me-3">
                    <a href="#" class="profile-picture link-body-emphasis text-decoration-none" data-bs-toggle="dropdown"
                        ...
                        <img id="adminProfilePic" src="https://github.com/mdo.png" alt="mdo" width="38" height="38"
        ...
        <div class="dashboard-main-content">
            ...
            <div class="dashboard-left-column">
                ...
                <div class="admin-card" id="aktivasiAkunCard">
                    <div class="title-arrow">
                        <h3>Aktivasi Akun Baru</h3>
                        {{-- Link ini akan kita buat fungsional dengan JavaScript --}}
                        <a href="#"><img src="{{ asset('storage/arrow.png') }}" alt="arrow"></a>
                    </div>
                    {{-- Sesuai permintaan, tidak ada isi list di dalamnya --}}
                </div>
            ...
            <div class="dashboard-right-column">
                ...
                <div class="admin-card" id="kelolaIuranCard">
                    <div class="title-arrow">
                        <h3>Kelola Iuran</h3>
                        <a href="#"><img src="{{ asset('storage/arrow.png') }}" alt="arrow"></a>
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            async function initDashboard() {
                ...
                const [mutasiResult, wargaResult, iuranResult, prokerResult, kritikResult, meResult] = results;
                ...
                if (iuranResult.status === 'fulfilled') {
                    const iuranData = iuranResult.value.data.data;
                    const sourceData = (userRole.includes('rt') ? iuranData.rt : iuranData.rw) || [];
                    const filteredIuran = sourceData.filter(item =>
                        userRole.includes('rt') ? item.rt_id === userRtId : item.rw_id === userRwId
                    );
                    processIuranData({ data: filteredIuran });
                } else {
                    console.error("Gagal mengambil data iuran:", iuranResult.reason);
                }
                ...
                if (kritikResult.status === 'fulfilled') {
                    ...
                    // Logika filter kini berdasarkan role admin dan rt_id tujuan kritik
                    const filteredKritik = userRole.includes('admin_rt') // Gunakan 'admin_rt' agar lebih spesifik
                        ? allKritik.filter(k => k.rt_id === userRtId) // Filter berdasarkan rt_id milik kritik
                        : allKritik; // Jika bukan Admin RT (misal: Admin RW), tampilkan semua
            ...
            function processIuranData(response) {
                const iuran = response.data || [];
                const iuranList = document.getElementById('iuranList');
                iuranList.innerHTML = '';

                if (iuran.length > 0) {
                    // BARU: Urutkan data berdasarkan yang terbaru (created_at) sebelum di-slice
                    const latestIuran = iuran
                        .sort((a, b) => new Date(b.created_at) - new Date(a.created_at))
                        .slice(0, 2);

                    latestIuran.forEach(item => {
                        const li = document.createElement('li');
                        li.innerHTML = `<span>${item.name}</span><span class="fw-bold">${formatCurrency(item.value)}</span>`;
                        iuranList.appendChild(li);
                    });
                } else {
                    iuranList.innerHTML = '<li>Belum ada jenis iuran.</li>';
                }
            }
            ...
            function setupCardNavigation() {
                ...
                addNavigation('aktivasiAkunCard', '{{ route("aktivasi-akun") }}');
                addNavigation('kelolaIuranCard', '{{ route("manajemen-iuran") }}');
