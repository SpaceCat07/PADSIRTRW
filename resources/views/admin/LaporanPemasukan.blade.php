...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchData() {
        ...
        try {
            ...
            // 3. Saring lagi untuk hanya menampilkan data pemasukan ('inflow')
            allData = idFilteredData.filter(item => item.variance === 'inflow');
            // --- AKHIR FILTER ---

            if (allData.length === 0) {
                tableBody.innerHTML = '<tr><td colspan="5" style="text-align: center;">Tidak ada data pemasukan ditemukan untuk Anda.</td></tr>';
            } else {
    ...
    function renderTable() {
        tableBody.innerHTML = "";
        if (allData.length === 0 && currentPage === 1) {
        ...
        pageData.forEach(item => {
            ...
            const formattedValue = new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0 }).format(item.value);
            row.innerHTML = `<td>${item.id}</td><td>${item.date}</td><td>${formattedValue}</td><td>${item.notes || "-"}</td><td><button class="btn-view" disabled>Lihat</button></td>`;
