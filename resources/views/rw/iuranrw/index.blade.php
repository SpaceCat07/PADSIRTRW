...
<a href="{{route('iuranRW.create')}}">Tambah Data Iuran RW</a>
    ...
    <tbody>
        @foreach ($iuranTambahan as $tambahan)
            <tr>
                ...
                <td>
                    <form action="{{route('iuranRW.destroy', $tambahan -> id)}}" method="post">
                        @csrf
                        @method('DELETE')
                ...
                <td>
                    <a href="{{route('iuranRW.edit', $tambahan -> id)}}">Edit data Iuran Tambahan</a>
                </td>
            </tr>
        @endforeach
    </tbody>
    ...
    <tbody>
        @foreach ($iuranBulanan as $bulanan)
            <tr>
                ...
                <td>
                    <form action="{{route('iuranRW.destroy', $bulanan -> id)}}" method="post">
                        @csrf
                        @method('DELETE')
                        <button type="submit" onclick="return confirm('Apakah anda yakin ingin menghapus data Iuran ini?')">Hapus Iuran RT</button>
                    </form>
